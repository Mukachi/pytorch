...
def call_delegate_cpu(lowered_module, original_gm, weight_args, input_args):
    ...
    has_fake_params = any(isinstance(param, FakeTensor) for param in original_gm.parameters())
    has_fake_buffers = any(isinstance(buffer, FakeTensor) for buffer in original_gm.buffers())
...
# pyre-ignore
def call_delegate_fake_tensor_mode(mode, lowered_module, original_gm, weight_args, input_args):
    with mode:
